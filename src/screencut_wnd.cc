...
namespace ui {
  ...
  using graphs::GraphFactory;
  using namespace Gdiplus;

  std::vector<RectX> CScreenCutWnd::m_vecAllWndRect;
  HCURSOR CScreenCutWnd::m_hCursor = NULL;
  ...
  LPCTSTR CScreenCutWnd::GetWindowClassName() const {
    return _T("CScreenCutWnd");
  }

  void CScreenCutWnd::CreateToolWnd() {
  ...
  void CScreenCutWnd::CreateColorWnd(BOOL bShow) {
    if (NULL != m_pColorWnd) {
  ...
  void CScreenCutWnd::InitializeDC() {
  ...
  BOOL CScreenCutWnd::AutoRecognizeWnd(const POINT& ptMousePos) {
  ...
  void CScreenCutWnd::Stretch_Adjust(const STRETCH& emStretch, RectX& rcStretch, int cxOffset, int cyOffset) {
    switch (emStretch) {
    default:
      break;
    case STRETCH_L:
  ...
  void CScreenCutWnd::Drag_Adjust(RectX& rcDrag, int cxOffset, int cyOffset) {
  ...
  BOOL CScreenCutWnd::Adjust(int cxOffset, int cyOffset) {
    //拉伸
    if (m_bStretching) {
  ...
  void CScreenCutWnd::AdjustToolPos() {
  ...
  void CScreenCutWnd::Enlarge(RectX& rcEnlarge, int iSpeed) {
  ...
  void CScreenCutWnd::PaintSelRgn() {
    m_hOldGrayBMP2 = (HBITMAP)::SelectObject(m_hMemDC, m_hGrayBMP2);
  ...
  BOOL CScreenCutWnd::Save() {
    BOOL bSuccess = FALSE;

    CString strDefName;
    SetDefFileName(strDefName);

    int nFilterIndex = m_nDefaultFilterIndex + 1;  //过滤器索引
  ...
  void CScreenCutWnd::ClearRedoStack() {
  ...
  LRESULT CScreenCutWnd::OnCreate(WPARAM wParam, LPARAM lParam) {
    GetAllWndRect();
  ...
  LRESULT CScreenCutWnd::OnPaint(WPARAM wParam, LPARAM lParam) {
    PAINTSTRUCT ps;
  ...
  LRESULT CScreenCutWnd::OnLButtonDown(WPARAM wParam, LPARAM lParam) {
  ...
  LRESULT CScreenCutWnd::OnMouseMove(WPARAM wParam, LPARAM lParam) {
    //设置光标，拉伸过程中，光标保持不变
  ...
  LRESULT CScreenCutWnd::OnLButtonUp(WPARAM wParam, LPARAM lParam) {
    POINT ptParam = { LOWORD(lParam), HIWORD(lParam) };
  ...
  LRESULT CScreenCutWnd::OnKeyDown(WPARAM wParam, LPARAM lParam) {
    if (VK_ESCAPE == wParam) {
  ...
  LRESULT CScreenCutWnd::OnMouseWheel(WPARAM wParam, LPARAM lParam) {
  ...
  LRESULT CScreenCutWnd::OnLButtonDbClk(WPARAM wParam, LPARAM lParam) {
    if (m_emAction > ACTION_CHOOSING) {
      SendMessage(SCMSG_FINISH);
    }
    return 0;
  ...
  LRESULT CScreenCutWnd::OnRButtonUp(WPARAM wParam, LPARAM lParam) {
    if (m_emAction == ACTION_CHOOSING) {
      SendMessage(WM_CLOSE);
    } else {
      POINT ptCurPos = { 0 };
  ...
  LRESULT CScreenCutWnd::OnDestroy(WPARAM wParam, LPARAM lParam) {
    ClearUndoStack();
    ClearRedoStack();
  ...
  LRESULT CScreenCutWnd::OnRecap(WPARAM wParam, LPARAM lParam) {
    ClearUndoStack();
    ClearRedoStack();
  ...
  LRESULT CScreenCutWnd::OnSaveImage(WPARAM wParam, LPARAM lParam) {
  ...
  LRESULT CScreenCutWnd::OnRectangle(WPARAM wParam, LPARAM lParam) {
  ...
  LRESULT CScreenCutWnd::OnEllipse(WPARAM wParam, LPARAM lParam) {
    BOOL bShow = BOOL(wParam);
  ...
  LRESULT CScreenCutWnd::OnArrowLine(WPARAM wParam, LPARAM lParam) {
    BOOL bShow = BOOL(wParam);
  ...
  LRESULT CScreenCutWnd::OnScrawl(WPARAM wParam, LPARAM lParam) {
    BOOL bShow = BOOL(wParam);
  ...
  LRESULT CScreenCutWnd::OnHighlight(WPARAM wParam, LPARAM lParam) {
    BOOL bShow = BOOL(wParam);
  ...
  LRESULT CScreenCutWnd::OnUndo(WPARAM wParam, LPARAM lParam) {
    Undo();
    InvalidateRgn(GetSafeHwnd(), NULL, FALSE);
  ...
  LRESULT CScreenCutWnd::OnRedo(WPARAM wParam, LPARAM lParam) {
    Redo();
    InvalidateRgn(GetSafeHwnd(), NULL, FALSE);
  ...
  LRESULT CScreenCutWnd::OnCancel(WPARAM wParam, LPARAM lParam) {
    SendMessage(WM_CLOSE, lParam, wParam);
    return 0;
  }

  LRESULT CScreenCutWnd::OnFinish(WPARAM wParam, LPARAM lParam) {
    Finish();
    return 0;
  }

  LRESULT CScreenCutWnd::OnPenSizeChange(WPARAM wParam, LPARAM lParam) {
    m_nPenWidth = wParam;
    return 0;
  }

  LRESULT CScreenCutWnd::OnPenColorChange(WPARAM wParam, LPARAM lParam) {
    m_dwPenColor = (COLORREF)wParam;
    return 0;
  }

  LRESULT CScreenCutWnd::ProcessMsg(UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    default:
      break;
    case WM_CREATE:
  ...
  void CScreenCutWnd::ConvertToGrayBitmap(HBITMAP hSourceBmp, HDC sourceDc) {
  ...
  BOOL CScreenCutWnd::BitBltEx(HDC hDestDC, RectX rc, HDC hSrcDC, POINT m_ptStart, DWORD dwRop) {
    return BitBlt(hDestDC, rc.leftX, rc.topX, rc.GetW(), rc.GetH(), hSrcDC, m_ptStart.x, m_ptStart.y, dwRop);
  }

  int CScreenCutWnd::DrawTextEx(HDC hDC, const CString& strTx, RectX rc, UINT uFormat) {
    RECT rcTemp = rc.ToRECT();
    return DrawText(hDC, strTx, lstrlen(strTx), &rcTemp, uFormat);
  }

  BOOL CScreenCutWnd::RectangleX(HDC hDC, RectX rc) {
    return Rectangle(hDC, rc.leftX, rc.topX, rc.rightX, rc.bottomX);
  }

  void CScreenCutWnd::DrawRect(HDC hDC, const RectX& rcDraw, const int& iPenWidth, const int& iPenStyle) {
    LOGBRUSH  LogBrush;
  ...
  void CScreenCutWnd::DrawAdjustSquare(HDC hDC, const RectX& rc, const int iSize) {
  ...
  void CScreenCutWnd::DrawSizeTx(HDC hDC, const RectX& m_rcSel) {
    CString strRcSize;
  ...
  BOOL CScreenCutWnd::InsertWndRect(HWND hwnd) {
  ...
  BOOL CScreenCutWnd::EnumGrandChildWindowsProc(HWND hwnd, LPARAM lParam) {
    InsertWndRect(hwnd);
    return TRUE;
  }

  BOOL CScreenCutWnd::EnumChildWindowsProc(HWND hwnd, LPARAM lParam) {
    ::EnumChildWindows(hwnd, &EnumGrandChildWindowsProc, NULL);
    return TRUE;
  }

  BOOL CScreenCutWnd::EnumWindowsProc(HWND hwnd, LPARAM lParam) {
    ::EnumChildWindows(hwnd, &EnumChildWindowsProc, NULL);
    InsertWndRect(hwnd);
    return TRUE;
  }

  std::vector<RectX> CScreenCutWnd::GetAllWndRect() {
  ...
  BOOL CScreenCutWnd::GetSaveFilePath(HINSTANCE hInstance, HWND hWnd, TCHAR* szFilePath, TCHAR* szFileTitle, int& nFilterIndex) {
    OPENFILENAME ofn;
  ...
  void CScreenCutWnd::SetScrnCursor(HWND hWnd, const RectX& rcCursorLie, const BOOL& bLButtonDown, const ACTION& m_emAction, STRETCH& emStretch) {
    if (NULL != m_pToolWnd) {
  ...
  void CScreenCutWnd::SetStretchCursor(const RectX& rc, const POINT& ptPos, STRETCH& emStretch) {
  ...
  STRETCH CScreenCutWnd::GetStrechDrct(const RectX& rc, const POINT& ptPos) {
  ...
  BOOL CScreenCutWnd::CopyBMP2Clipboard(HBITMAP hBMP, HWND hWnd) {
  ...
  BOOL CScreenCutWnd::GetEncoderClsid(const WCHAR* format, CLSID* pClsid) {
  ...
  BOOL CScreenCutWnd::SaveAsPic(HBITMAP hImage, const CString& pFileName, const CString& strFormat) {
    BOOL bRes = FALSE;
    ...
    {
      CLSID picClsid;
      CScreenCutWnd::GetEncoderClsid(strFormat, &picClsid);
